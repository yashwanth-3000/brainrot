...
class InMemoryRepository(Repository):
    def __init__(self) -> None:
        ...
        self._short_engagements: dict[str, ShortEngagementRecord] = {}
    ...
    async def upsert_short_engagement(
        ...
        async with self._lock:
            existing = self._short_engagements.get(engagement.session_id)
            if existing is not None:
                engagement = engagement.model_copy(update={"id": existing.id, "created_at": existing.created_at})
            self._short_engagements[engagement.session_id] = engagement
            return engagement

    async def list_short_engagements(self, chat_id: str) -> list[ShortEngagementRecord]:
        engagements = [record for record in self._short_engagements.values() if record.chat_id == chat_id]
        return sorted(engagements, key=lambda record: record.updated_at, reverse=True)
