...
function toLogEntry(event: BatchEventRecord): LogEntry {
  ...
  if (event.type === "log") {
    ...
    if (title.startsWith("Waiting for ElevenLabs narration audio for item ")) {
      return { ...base, iconKey: "elevenlabs", providerLabel: "ElevenLabs Agent", title: `The ElevenLabs narrator agent is still speaking ${videoLabelFromPayload(payload)}`, detail: withEventMeta("A live ElevenLabs agent conversation is still producing speech audio for this script before forced alignment can begin.", event) };
    }
  ...
  if (event.type === "scripts_ready") {
    const mode = stringValue(payload.mode);
    return { ...base, tone: "success", iconKey: "success", providerLabel: producerProviderLabel(mode), title: "Script generation is complete", detail: withEventMeta(`${payload.script_count ?? 0} scripts cleared local QA after ${payload.attempt_count ?? 0} generation attempt${Number(payload.attempt_count) === 1 ? "" : "s"} and ${payload.repair_count ?? 0} repair pass${Number(payload.repair_count) === 1 ? "" : "es"}. ${mode === "elevenlabs_native" ? "These scripts came from the ElevenLabs producer agent." : "These scripts came from OpenAI generation."}`, event) };
  }
  if (event.type === "narrator_conversation_started") {
    return { ...base, iconKey: "elevenlabs", providerLabel: "ElevenLabs Agent", title: `An ElevenLabs narrator agent conversation started for ${videoLabelFromPayload(payload)}`, detail: withEventMeta("The approved script is now being injected into a live ElevenLabs agent conversation so subtitle timing can be built from real audio.", event) };
  }
